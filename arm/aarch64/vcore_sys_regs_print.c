...
extern sys_reg_info_t sys_regs[];

static const char* SPACER = "                                                                                ";


static const char* decode_nibble_at(u64 value, int bits, const char* desc[])
{
    return desc[((value >> bits) & 0x0F)];
}

static int not_implemented(sys_reg_t reg, u64 value, int spacing, detail_t detail)
{
    if (detail >= FULL) {
        spacing +=4;
        fprintf(stderr, "%.*s<no detail print implemented>\n", spacing, SPACER);
    }
    return 0;
}
...
static int iss_decode(uint32_t class, uint32_t iss, int spacing, detail_t detail)
{
    switch(class) {
        ...
        case BREAKPOINT_EXCEPTION: { // Trapped MSR, MRS or System instruction execution in AArch64 state, that is not reported using EC 0b000000, 0b000001, or 0b000111.
            ...
            fprintf(stderr, "%.*sBreakppoint Exception #%d\n", spacing, SPACER, imm16);
        }
        break;
        ...
        case DATA_ABORT_EXCEPTION: { // Data Abort from a lower Exception level.
            ...
            if (is_valid) {
                ...
                fprintf(stderr, "%.*sSyndrome Access Size=%s\n", spacing, SPACER, sas_desc[sas]);
                fprintf(stderr, "%.*slen=%x\n", spacing, SPACER, len);
                fprintf(stderr, "%.*sSyndrome Register Transfer=x%d\n", spacing, SPACER, srt);
                fprintf(stderr, "%.*scm=%x\n", spacing, SPACER, cm);
                fprintf(stderr, "%.*sData Fault Status Code=%s\n", spacing, SPACER, dfsc_desc[dfsc]);
            }
        }
        break;
    };
...
static const char* __decode_isa_at(u64 value, int bits, const char* isa, int spacing, detail_t detail, const char* desc[], int ndescs)
{
    int index= ((value >> bits) & 0x0F);
    if (index == 0) return "";
    if (index > ndescs)
        fprintf(stderr, "%.*s%s: invalid index %d\n", spacing, SPACER, isa, index);
    else
...
int isar0_el1(sys_reg_t reg, u64 value, int spacing, detail_t detail)
{
    if (detail >= FULL) {
        spacing +=4;
        DECODE_ISA_AT(value, 4, "AES", spacing, detail, ((const char* []){
            "1, AESE, AESD, AESMC, and AESIMC.",
            "2, AESE, AESD, AESMC, AESIMC and PMULL/PMULL2."
        }));
        DECODE_ISA_AT(value, 8, "SHA1", spacing, detail, ((const char* []){
            "1, SHA1C, SHA1P, SHA1M, SHA1H, SHA1SU0, and SHA1SU1",
        }));
        DECODE_ISA_AT(value, 12, "SHA2", spacing, detail, ((const char* []){
        ...
        DECODE_ISA_AT(value, 16, "CRC32", spacing, detail, ((const char* []){
            "1, CRC32B, CRC32H, CRC32W, CRC32X, CRC32CB, CRC32CH, CRC32CW, and CRC32CX.",
        }));
        DECODE_ISA_AT(value, 20, "Atomic", spacing, detail, ((const char* []){
            "1, LDADD, LDCLR, LDEOR, LDSET, LDSMAX, LDSMIN, LDUMAX, LDUMIN, CAS, CASP, and SWP.",
        }));
        DECODE_ISA_AT(value, 28, "RDM", spacing, detail, ((const char* []){
            "1, SQRDMLAH and SQRDMLSH.",
        }));
        DECODE_ISA_AT(value, 32, "SHA3", spacing, detail, ((const char* []){
            "1, EOR3, RAX1, XAR, and BCAX.",
        }));
        DECODE_ISA_AT(value, 36, "SM3", spacing, detail, ((const char* []){
            "1, SM3SS1, SM3TT1A, SM3TT1B, SM3TT2A, SM3TT2B, SM3PARTW1, and SM3PARTW2.",
        }));
        DECODE_ISA_AT(value, 40, "SM4", spacing, detail, ((const char* []){
            "1, SM4E and SM4EKEY.",
        }));
        DECODE_ISA_AT(value, 44, "DP - Dot Product", spacing, detail, ((const char* []){
            "1, UDOT and SDOT.",
        }));
        DECODE_ISA_AT(value, 48, "FHM", spacing, detail, ((const char* []){
            "1, FMLAL and FMLSL.",
        }));
        DECODE_ISA_AT(value, 52, "TS", spacing, detail, ((const char* []){
            "1, FCFINV, RMIF, SETF16, and SETF8.",
            "2, CFINV, RMIF, SETF16, SETF8, AXFLAG, and XAFLAG."
        }));
        DECODE_ISA_AT(value, 56, "TLB", spacing, detail, ((const char* []){
        ...
        DECODE_ISA_AT(value, 60, "RNDR", spacing, detail, ((const char* []){
            "1, RNDR and RNDRRS .",
        }));
    }
    return 0;
...
int isar1_el1(sys_reg_t reg, u64 value, int spacing, detail_t detail)
{
    if (detail >= FULL) {
        spacing +=4;
        DECODE_ISA_AT(value, 0, "DP - Data Persistence writeback", spacing, detail, ((const char* []){
            "1, DC CVAP.",
            "2, DC CVAP and DC CVADP."
        }));
        DECODE_ISA_AT(value, 4, "APA - DQARMA5 algorithm", spacing, detail, ((const char* []){
        ...
        DECODE_ISA_AT(value, 8, "API - IMPLEMENTATION DEFINED algorithm is implemented in the PE for address authentication", spacing, detail, ((const char* []){
        ...
        DECODE_ISA_AT(value, 12, "JSCVT - JavaScript conversion from double precision floating point values to integers", spacing, detail, ((const char* []){
            "1, FJCVTZS .",
        }));
        DECODE_ISA_AT(value, 16, "FCMA", spacing, detail, ((const char* []){
            "1, FCMLA and FCADD.",
        }));
        DECODE_ISA_AT(value, 20, "LRCPC", spacing, detail, ((const char* []){
            "1,  LDAPUR*, and STLUR*.",
            "2, LDAPR*, LDAPUR*, and STLUR*",
        }));
        DECODE_ISA_AT(value, 24, "GPA", spacing, detail, ((const char* []){
            "1, Generic Authentication using the QARMA5 algorithm is implemented. This includes the PACGA instruction.",
        }));
        DECODE_ISA_AT(value, 28, "GPI", spacing, detail, ((const char* []){
            "1, Generic Authentication using an IMPLEMENTATION DEFINED algorithm is implemented. This includes the PACGA instruction.",
        }));
        DECODE_ISA_AT(value, 32, "FRINTTS", spacing, detail, ((const char* []){
            "1, FRINT32Z, FRINT32X, FRINT64Z, and FRINT64X.",
        }));
        DECODE_ISA_AT(value, 36, "SB", spacing, detail, ((const char* []){
            "1, SB.",
        }));
        DECODE_ISA_AT(value, 40, "SPECRES", spacing, detail, ((const char* []){
            "1, CFP RCTX, DVP RCTX, and CPP RCTX.",
        }));
        DECODE_ISA_AT(value, 44, "BF16", spacing, detail, ((const char* []){
            "1, BFCVT, BFCVTN, BFCVTN2, BFDOT, BFMLALB, BFMLALT, and BFMMLA.",
        }));
        DECODE_ISA_AT(value, 48, "DGH", spacing, detail, ((const char* []){
            "1, Data Gathering Hint is implemented.",
        }));
        DECODE_ISA_AT(value, 52, "I8MM - Advanced SIMD and Floating-point Int8 matrix multiplication", spacing, detail, ((const char* []){
            "1, SMMLA, SUDOT, UMMLA, USMMLA, and USDOT.",
        }));
        DECODE_ISA_AT(value, 56, "XS", spacing, detail, ((const char* []){
        ...
        DECODE_ISA_AT(value, 60, "LS64", spacing, detail, ((const char* []){
            "1, LD64B and ST64B.",
...
// ------------------------------
// MAIR
static const char* dev_attributes[] = {
...
static const char* xsdev_attributes[] = {
...
static const char* normal_attributes[] = {
    "unpredictable",
...
static const char* normal_attributes2[] = {
    "unpredictable",
...
static const char* decode_mair_at(u64 value, int i) {
...
int pfr0_el1(sys_reg_t reg, u64 value, int spacing, detail_t detail)
{
    if (detail >= FULL) {
        ...
        fprintf(stderr, "%.*sSpeculative use of out of context branch targets (CSV2): %s\n", spacing, SPACER, decode_nibble_at(value, 56, (const char* []){
        ...
        fprintf(stderr, "%.*sSpeculative use of out of context branch targets (CSV3): %s\n", spacing, SPACER, decode_nibble_at(value, 60, (const char* []){
...
int pfr1_el1(sys_reg_t reg, u64 value, int spacing, detail_t detail)
{
    if (detail >= FULL) {
        spacing +=4;
        fprintf(stderr, "%.*sBranch Target Identification: %s\n", spacing, SPACER, decode_nibble_at(value, 0, generic_desc));
        fprintf(stderr, "%.*sSpeculative Store Bypassing controls in AArch64 state: %s\n", spacing, SPACER, decode_nibble_at(value, 4, (const char* []){
            "Not implemented",
...
#define BIT_AT(n, bitname, v1, v2) fprintf(stderr, "%.*s%s(%d): %s\n", spacing, SPACER, bitname, n, ((value >> 0) & 1) != 0 ? v1 : v2)
...
#define KVM_REG_FROM_OFFSET(template, x)  (((u32*)&(x)) - ((u32*)&template))

int vcore_print_general_regs(struct kvm_cpu* vcore, int spacing)
{
	struct kvm_one_reg reg;
	int i, j;
	struct kvm_regs regs_template;
	for (i=0; i<4; i++) {
		fprintf(stderr, "%.*s", spacing, SPACER);
		for (j=0; j<7; j++) {
			int reg_index = i*7+j;
			reg.addr = (u64)&regs_template.regs.regs[reg_index];
			reg.id	= KVM_REG_FROM_OFFSET(regs_template, regs_template.regs.regs[reg_index]);
			if (ioctl(vcore->vcpu_fd, KVM_GET_ONE_REG, &reg) < 0)
				die_perror("KVM_SET_ONE_REG failed (x1)");
			fprintf(stderr, reg_index < 10 ? FORMAT_1DIGIT : FORMAT_2DIGITS, reg_index, regs_template.regs.regs[reg_index]);
		}
		fprintf(stderr, "\n");
	}

	reg.addr = (u64)&regs_template.regs.sp;
	reg.id	= KVM_REG_FROM_OFFSET(regs_template, regs_template.regs.sp);
	if (ioctl(vcore->vcpu_fd, KVM_GET_ONE_REG, &reg) < 0)
		die_perror("KVM_SET_ONE_REG failed (x1)");

	reg.addr = (u64)&regs_template.regs.pc;
	reg.id	= KVM_REG_FROM_OFFSET(regs_template, regs_template.regs.pc);
	if (ioctl(vcore->vcpu_fd, KVM_GET_ONE_REG, &reg) < 0)
		die_perror("KVM_SET_ONE_REG failed (x1)");

	reg.addr = (u64)&regs_template.regs.pstate;
	reg.id	= KVM_REG_FROM_OFFSET(regs_template, regs_template.regs.pstate);
	if (ioctl(vcore->vcpu_fd, KVM_GET_ONE_REG, &reg) < 0)
		die_perror("KVM_SET_ONE_REG failed (x1)");

	fprintf(stderr, "%.*s FP=%016llx PC=%016llx\n", spacing, SPACER,
		regs_template.regs.sp,
		regs_template.regs.pc
		);
	fprintf(stderr, "%.*sCPSR=%016llx\n", spacing, SPACER,
		regs_template.regs.pstate);
	return 0;
}
...
int vcore_print_sys_regs(struct kvm_cpu* vcore, int spacing, detail_t detail)
{
	...
	for(i = 0; i < register_count; i++) {
		if (sys_regs[i].id == 0) continue; // we hit the marker
		u64 value=0;
		if (sys_regs[i].minimal_el > 1) {
			//TODO: registers values per vcore....value = sys_regs[i].value;
		}
		else {
			value = vcore_get_sysreg (vcore, sys_regs[i].id);
		}
